...
class FG_Parser(object):
	...
	def get_ids_csv(self, pit=True, bat=True):
		if not pit and not bat:
			...
			return 
		if pit and self.pitcher_csv not in os.listdir(self.csv_dir):
			self.log('{} not found'.format(self.pitcher_csv), ex=True)
		if bat and self.batter_csv not in os.listdir(self.csv_dir):
			self.log('{} not found'.format(self.batter_csv), ex=True)
		if not (bat and pit):
			file_name = '{}{}'.format(self.csv_dir, self.pitcher_csv if pit else self.batter_csv)
			ids = dict()
	...
	def get_ids_web(self, pit=True, bat=True, clean=False):
		if not pit and not bat:
			...
			return
		if pit and self.pitcher_csv in os.listdir(self.csv_dir) and not clean:
			self.log('Pitcher ID#s already found in: {}'.format(self.pitcher_csv))
			self.get_ids_csv(True, False)
		if bat and self.batter_csv in os.listdir(self.csv_dir) and not clean:
		...
		if not (bat and pit):
			...
			p_csv = '{}{}'.format(self.csv_dir, self.pitcher_csv if pit else self.batter_csv)
	...
	def get_game_logs(self, player, clean=False):
		...
		file_name = '{}{}'.format(self.csv_dir, csv_name)
		# Make sure that the file does not exist
		if csv_name in os.listdir(self.csv_dir) and not clean:
		...
		self.log('Game logs for {} written to: {}'.format(p_tup[1], file_name))
		return file_name
